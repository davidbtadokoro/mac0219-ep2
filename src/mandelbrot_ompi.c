...
void init_mpi(){
    ...
    procs_with_one_more_pos = (image_buffer_size % (num_procs-1));

    if (rank <= procs_with_one_more_pos) {
        task_size = (image_buffer_size / (num_procs-1)) + 1;
        beg_index = (rank-1) * task_size;
        end_index = beg_index + task_size;
    }
    else {
        task_size = (image_buffer_size / (num_procs-1));
        beg_index = procs_with_one_more_pos * (task_size+1);
        beg_index += (rank-1-procs_with_one_more_pos) * task_size;
        end_index = beg_index + task_size;
    };
    ...
    // If it is the master
        ...
        for (i = 1; i < num_procs; i++) {
            ...
            if (worker_rank <= procs_with_one_more_pos) {
                task_size = (image_buffer_size / (num_procs-1)) + 1;
                beg_index = (worker_rank-1) * task_size;
            }
            else {
                task_size = (image_buffer_size / (num_procs-1));
                beg_index = procs_with_one_more_pos * (task_size+1);
                beg_index += (worker_rank-1-procs_with_one_more_pos) * task_size;
            };
...
int main(int argc, char *argv[]){
    ...
    /* Distributed block */
    init_mpi();
    if (rank) { compute_mandelbrot(beg_index, end_index); };
    term_mpi();
